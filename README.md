...
| ------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| **RF01**      | O sistema deve permitir que o advogado cadastre informações básicas de um processo jurídico, incluindo número do processo, partes envolvidas (autor, réu e advogados), data de abertura, status inicial (ativo, arquivado ou concluído), tipo do processo e comarca. | **Must**   |
| **RF02**      | O sistema deve permitir que o advogado ou perito faça upload de documentos relacionados a um processo em formatos como PDF, DOCX, JPEG e PNG, vinculando-os diretamente ao cadastro do processo.                                                                     | **Must**   |
| **RF03**      | O sistema deve permitir que o advogado consulte todos os processos cadastrados, filtrando por status (ativo, arquivado e concluído), tipo de processo e período.                                                                                                     | **Must**   |
...
| **RF05**      | O sistema deve enviar notificações para o advogado sobre prazos processuais, com aviso padrão de 7 dias antes do vencimento, podendo ser configurado pelo usuário para entre 1 e 30 dias.                                                                            | **Should** |
...
| **RF08**      | O sistema deve realizar análise de documentos jurídicos por meio de inteligência artificial, destacando trechos relevantes, cláusulas, prazos e dados sensíveis, além de sugerir padrões recorrentes.                                                                | **Must**   |
| **RF09**      | O sistema deve permitir que o administrador gerencie permissões de acesso de forma granular, incluindo advogados, peritos, estagiários e outros colaboradores.                                                                                                       | **Must**   |
...
| **RNF02**     | O sistema deve utilizar um banco de dados não relacional (NoSQL) para armazenar documentos, anotações e metadados dos processos, garantindo escalabilidade em volume de dados, especialmente documentos e históricos de processos.               | RF02, RF06, RF08       | **Must**   |
| **RNF03**     | O sistema deve oferecer suporte à concorrência e alta performance, utilizando a linguagem Go para otimizar tarefas paralelas, como geração de relatórios, processamento de documentos e análise com IA.                                          | RF02, RF04, RF05, RF08 | **Should** |
...
| **RNF05**     | As notificações de prazos devem ser entregues com latência inferior a 5 minutos, via e-mail, push notification ou SMS.                                                                                                                           | RF05                   | **Should** |
...
| **RNF08**     | A análise com inteligência artificial deve processar documentos com tempo de resposta inferior a 10 segundos por arquivo, exibindo trechos destacados e insights com acurácia mínima de 90%, utilizando modelos treinados em linguagem jurídica. | RF08                   | **Must**   |
...
**Viabilidade Técnica:**
Quanto à viabilidade técnica, todos os aspectos são simples, com exceção da IA e do algoritmo de censura de dados sensíveis. Para a IA, será utilizado um serviço de terceiros (OpenAI, Claude, etc.), e o algoritmo pode ser importado de bibliotecas abertas para garantir viabilidade e segurança.
